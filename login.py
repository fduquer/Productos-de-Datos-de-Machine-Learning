...
from streamlit_cookies_controller import (
    CookieController,  # Librería para manejar cookies en Streamlit. Instalación: pip install streamlit-cookies-controller
)
...
def validarUsuario(usuario, clave):
    ...
    dfusuarios = pd.read_csv("access/usuarios.csv")
    # Filtramos el dataframe para buscar el usuario y la clave
    if (
        len(
            dfusuarios[
                (dfusuarios["usuario"] == usuario) & (dfusuarios["clave"] == clave)
            ]
        )
        > 0
    ):
...
def generarMenu(usuario):
    ...
    with st.sidebar:  # Creamos una barra lateral para el menú
        ...
        st.write(
            f"Hola **:blue-background[{nombre}]** "
        )  # Mostramos el nombre del usuario con formato
        st.caption(f"Rol: {rol}")  # Mostramos el rol del usuario
        # Mostramos los enlaces de páginas
        st.page_link(
            "inicio.py", label="Inicio", icon=":material/home:"
        ...
        if rol in ["ventas", "admin", "comercial"]:
            st.page_link(
                "pages/paginaVentas.py", label="Ventas", icon=":material/sell:"
            )  # Enlace a la página de ventas
        if rol in ["compras", "admin", "comercial"]:
            st.page_link(
                "pages/paginaCompras.py",
                label="Compras",
                icon=":material/shopping_cart:",
            )  # Enlace a la página de compras
        if rol in ["personal", "admin", "compras"]:
            st.page_link(
                "pages/paginaPersonal.py", label="Personal", icon=":material/group:"
            )  # Enlace a la página de personal
        if rol in ["contabilidad", "admin"]:
            st.page_link(
                "pages/paginaContabilidad.py",
                label="Contabilidad",
                icon=":material/payments:",
            )  # Enlace a la página de contabilidad
...
def validarPagina(pagina, usuario):
    ...
    dfusuarios = pd.read_csv("access/usuarios.csv")
    dfPaginas = pd.read_csv("access/rol_paginas.csv")
    dfUsuario = dfusuarios[(dfusuarios["usuario"] == usuario)]
    rol = dfUsuario["rol"].values[0]
    ...
    if len(dfPagina) > 0:
        if (
            rol in dfPagina["roles"].values[0]
            or rol == "admin"
            or st.secrets["tipoPermiso"] == "rol"
        ):
...
def generarMenuRoles(usuario):
    ...
    with st.sidebar:  # Menú lateral
        ...
        if (
            st.secrets["ocultarOpciones"] == "True"
        ):  # Verificamos el valor del secreto "ocultarOpciones"
            ...
            for index, row in dfPaginas.iterrows():
                icono = row["icono"]
                st.page_link(
                    row["pagina"], label=row["nombre"], icon=f":material/{icono}:"
                )  # Mostramos la página
        else:  # Si no se ocultan las opciones
            # Deshabilitamo las páginas que no tiene permiso
            for index, row in dfPaginas.iterrows():
                deshabilitarOpcion = (
                    True  # Valor por defecto para deshabilitar las opciones
                )
                if rol in row["roles"] or rol == "admin":  # Verificamos el rol
                    deshabilitarOpcion = (
                        False  # Habilitamos la página si el usuario tiene permiso
                    )
...
def generarLogin(archivo):
    ...
    # Validamos si el usuario ya fue ingresado
    if (
        "usuario" in st.session_state
    ):  # Verificamos si la variable usuario esta en el session state
        # Si ya hay usuario cargamos el menu
        if st.secrets["tipoPermiso"] == "rolpagina":
            generarMenuRoles(
                st.session_state["usuario"]
            )  # Generamos el menú para la página
        else:
            generarMenu(st.session_state["usuario"])  # Generamos el menú del usuario
        if (
            validarPagina(archivo, st.session_state["usuario"]) == False
        ):  # Si el usuario existe, verificamos la página
            st.error(
                f"No tiene permisos para acceder a esta página {archivo}",
        ...
        with st.form("frmLogin"):  # Creamos un formulario de login
            parUsuario = st.text_input(
                "Usuario"
            )  # Creamos un campo de texto para usuario
            parPassword = st.text_input(
                "Password", type="password"
            )  # Creamos un campo para la clave de tipo password
            btnLogin = st.form_submit_button(
                "Ingresar", type="primary"
            )  # Botón Ingresar
            if btnLogin:  # Verificamos si se presiono el boton ingresar
                if validarUsuario(
                    parUsuario, parPassword
                ):  # Verificamos si el usuario y la clave existen
                    st.session_state["usuario"] = (
                        parUsuario  # Asignamos la variable de usuario
                    )
                    # Set a cookie
                    ...
                    st.error(
                        "Usuario o clave inválidos", icon=":material/gpp_maybe:"
